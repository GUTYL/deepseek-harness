...
describe('PiAiAdapter provider routing', () => {
  ...
  it('stamps the live session id over a profile header of the same name', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url, {
      headers: { 'x-deepseek-harness-session-id': 'stale-profile-value', 'x-opencode-session': 'stale' },
    })
    await assemble(ctx, {
      model: 'deepseek-v4-flash',
      messages: [],
      sessionId: 'live-session' as never,
    })
    expect(server.headers[0]?.['x-deepseek-harness-session-id']).toBe('live-session')
    expect(server.headers[0]?.['x-opencode-session']).toBe('live-session')
  })
